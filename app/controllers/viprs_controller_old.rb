...
#       end
 ...
 #     @vipr_item = @vipr_item2
       ...
       @po.each do |po|
        ...
        @vipr.grand_total = po.grand_total


        params[:totbefdisc] = po.totbefdisc
        @vipr.totbefdisc = po.totbefdisc
        ...
        params[:net_payment] = po.grand_total -  po.net_payment 
        @vipr.net_payment = po.grand_total -  po.net_payment 
        


        params[:dp_amount] = po.grand_total -  po.net_payment 
        @vipr.dp_amount = po.grand_total -  po.net_payment 
...
#           if @vipr.subtotal < PrApproval.find_by(requester: @vipr.req_email).apv2_amount
              #@vipr.approval2 = PrApproval.find_by(requester: current_user.email).apv2
#              @@apv2 = PrApproval.find_by(requester: current_user.email).apv2
  ...
  private
    ...
    def vipr_params
       ...
       if params[:action] == "update"
         ...
         params.require(:vipr).permit(:id,:disc, :pic,:dp,:net_payment,:dp_amount, :no,:po_no,:is_vat,:status, :supplier,:doctype, :invoice_no, :invoice_date, :due_date, :payment_no, :currency, :requester, :approval1, :apv_date1, :approval2, :apv_date2, :approval3, :apv_date3, :approval4, :apv_date4, :subtotal, :vat, :total , :remarks, attachment1:[], :vipr_items_attributes => [:vipr_id, :product, :no,:dept, :description, :spesification, :req_qty, :costcenter, :price, :quantity,  :amount, :_destroy,:id] )
       end
    end
end
